...
function json_result_formatter(results, start_date, server) {
    final_json = {}
    final_json["annotations"] = {"platform": os.platform()};
...
/*function find_ndt_server(callback) {
    request.get('http://mlab-ns.appspot.com/ndt?format=json', (error, response, body) => {
        if (!error && response.statusCode == 200) {
            var importedJSON = JSON.parse(body);
            var mlab_server_metro = importedJSON.url;
            var parsed_url = url.parse(mlab_server_metro, true);
            var clientNDTWrapper = new NDTWrapper(parsed_url.hostname, 3001, generate_callback())
...
/*function find_ndt_server(callback) {
    getJSON('http://mlab-ns.appspot.com/ndt?format=json', function (err, response) {
        if (err) {
            console.log("error retrieving mlab server list: "+err);
            process.exit(144);
        }
        var parsed_url_hostname = url.parse(response.url, true).hostname;
        var clientNDTWrapper = new NDTWrapper(parsed_url_hostname, 3001, generate_callback());
        clientNDTWrapper.run_test();
        return;
    });
}*/
        ...
        response.on("end", function(){ 
            if(response.statusCode === 200){ 
                try{ var profile = JSON.parse(body); 
                    ...
                    var clientNDTWrapper = new NDTWrapper(parsed_url_hostnamename, 3001, generate_callback())
                    clientNDTWrapper.run_test()
                } catch(error){ 
